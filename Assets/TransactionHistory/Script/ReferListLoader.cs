...
public class ReferListLoader : MonoBehaviour
{
    ...
    [SerializeField]
    private List<ReferList> TransactionKeys = new List<ReferList>();
    ...
    void OnFillItem(int index, GameObject item)
    {
        ...
        if (item.GetComponent<ReferListViewController>() != null)
        {
            if(TransactionKeys != null)
            {
                if (index < TransactionKeys.Count)
                {
                    item.GetComponent<ReferListViewController>().DisplayInfo(TransactionKeys[index]);
                }
            }
    ...
    public void APICall_First()
    {
        if (PlayerSave.singleton != null)
        {
            ...
            PlayerSave.singleton.GetReferDetails(PlayerSave.singleton.newID(), pageIndex.ToString(), pageSize.ToString(), OnListLoaded); 
    ...
    public void APICall_Second()
    {
        if (PlayerSave.singleton != null)
        {
            ...
            PlayerSave.singleton.GetReferDetails(PlayerSave.singleton.newID(), pageIndex.ToString(), pageSize.ToString(), OnListLoadedSecond);
        }
    }
    ...
    public void OnListLoaded(GetReferDetails _callback)
    {
        ...
        if(_callback!=null)
        {
            if (_callback.status.Equals("200"))
            {
                ...
                {
                    if(_callback.data.referList!=null)
                    {
                        payMentCount = _callback.data.referList.Length;
                    }
                ...
                for (int i = 0; i < payMentCount; i++)
                {
                    ...
                    if (_callback.message.Equals("OK"))
                    {
                        ...
                        AddTransactionKey(_callback.data.referList[i]);
                    }
                    else
                    {
                        TransactionLoaded--;
    ...
    public void OnListLoadedSecond(GetReferDetails _callback)
    {
    ...
    private void AddTransactionKey(ReferList _key)
    {
