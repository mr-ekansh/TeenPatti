...
public class HistoryListLoader : MonoBehaviour
{
    ...
    [SerializeField]
    private List<DepositList> TransactionKeys = new List<DepositList>();
    ...
    void OnFillItem(int index, GameObject item)
    {
        ...
        if (item.GetComponent<HistoryListViewController>() != null)
        {
            if(TransactionKeys != null)
            {
                if (index < TransactionKeys.Count)
                {
                    item.GetComponent<HistoryListViewController>().DisplayInfo(TransactionKeys[index]);
                }
            }
    ...
    public void APICall_First()
    {
        if (PlayerSave.singleton != null)
        {
            ...
            PlayerSave.singleton.GetDepositDetails(PlayerSave.singleton.newID(), pageIndex.ToString(), pageSize.ToString(), OnListLoaded); 
    ...
    public void APICall_Second()
    {
        if (PlayerSave.singleton != null)
        {
            ...
            PlayerSave.singleton.GetDepositDetails(PlayerSave.singleton.newID(), pageIndex.ToString(), pageSize.ToString(), OnListLoadedSecond);
        }
    }
    ...
    public void OnListLoaded(GetDepositDetails _callback)
    {
        ...
        if(_callback!=null)
        {
            if (_callback.status.Equals("200"))
            {
                ...
                else if (payMentCount <= 0)
                {
                    if (MessageLabel)
                    {
                        MessageLabelValue = "No transactions made yet...";
    ...
    public void OnListLoadedSecond(GetDepositDetails _callback)
    {
    ...
    private void AddTransactionKey(DepositList _key)
    {
