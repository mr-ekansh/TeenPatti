...
#pragma warning restore 0414
        ...
        private IEnumerator Start()
        {
            ...
            RequestMobileNoHint((mobileNo, status) => {
                RemoveHintAction();
                if (status == (int)AuthErrorCode.AUTH_HINTS_AVAILABLE)
                {
                    Debug.Log("mobile number is " + mobileNo);
                }
                else
                {
                    Debug.Log("mobile number is not found " + status);
                }
            });
        }
        ...
        private void Awake()
        {
            ...
            Debug.Log("my context is " + mContext);
            if (mContext == null)
            {
                Debug.Log("my context is null");
            }
            else
            {
                Debug.Log("my context is not null");
            }
#elif UNITY_EDITOR
        ...
        public void RequestMobileNoHint(Action<string, int> OnMobileHintAction)
        {
            ...
            using (AndroidJavaClass jc = new AndroidJavaClass("com.onedevapp.smartcredentials.AuthManager"))
            {
                ...
                {
                    Debug.Log("my authmanager is null");
                }
                else
                {
                    Debug.Log("my authmanager is not null");
                }
            }
#elif UNITY_EDITOR
            if (DebugMode)
